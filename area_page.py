...
class ServiceAreaEditDialog(QDialog):
    ...
    def _build_ui(self):
        ...
        title = QLabel("🏗 编辑服务区" if self.area else "🏗 添加服务区")
...
class ServiceAreaPage(QWidget):
    ...
    def __init__(self, parent=None):
        super().__init__(parent)
        self.areas = load_json(SERVICE_AREAS_FILE, DEFAULT_SERVICE_AREAS)
        self._build_ui()
        self._refresh_table()
    ...
    def _build_ui(self):
        ...
        self.table.setHorizontalHeaderLabels(["服务区名称", "X 坐标", "Y 坐标", "Z 坐标"])
    ...
    def _add(self):
        dlg = ServiceAreaEditDialog(self)
        if dlg.exec_() == QDialog.Accepted and dlg.result_data:
            self.areas.append(dlg.result_data)
            save_json(SERVICE_AREAS_FILE, self.areas)
            self._refresh_table()
    ...
    def _edit(self):
        ...
        if row < 0:
            ...
            return
        dlg = ServiceAreaEditDialog(self, area=self.areas[row])
        if dlg.exec_() == QDialog.Accepted and dlg.result_data:
            self.areas[row] = dlg.result_data
            save_json(SERVICE_AREAS_FILE, self.areas)
            self._refresh_table()
